...
ChangeType::ChangeType(ros::NodeHandle &t_nh)
{
    ...
    pub_roll = nh_.advertise<geometry_msgs::PoseStamped>("roll_angle", 100000);
    pub_pitch = nh_.advertise<geometry_msgs::PoseStamped>("pitch_angle", 100000);
    pub_yaw = nh_.advertise<geometry_msgs::PoseStamped>("yaw_angle", 100000);
}
